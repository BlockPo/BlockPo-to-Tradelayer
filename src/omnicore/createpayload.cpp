...
std::vector<unsigned char> CreatePayload_CreateContract(uint8_t ecosystem, uint32_t numType, uint32_t denomType, std::string name, uint32_t blocks_until_expiration, uint32_t notional_size, uint32_t collateral_currency, uint32_t margin_requirement, uint64_t ticksize)
{
    std::vector<unsigned char> payload;
    ...
    std::vector<uint8_t> vecMessageVer = CompressInteger(messageVer);
    std::vector<uint8_t> vecEcosystem = CompressInteger(ecosystem);
    std::vector<uint8_t> vecNumType = CompressInteger(numType);
    std::vector<uint8_t> vecDenomType = CompressInteger(denomType);
    ...
    std::vector<uint8_t> vecTickSize = CompressInteger(ticksize);

    if ((name).size() > 255) name = name.substr(0,255);
    payload.insert(payload.end(), vecMessageVer.begin(), vecMessageVer.end());
    payload.insert(payload.end(), vecMessageType.begin(), vecMessageType.end());
    payload.insert(payload.end(), vecEcosystem.begin(), vecEcosystem.end());
    payload.insert(payload.end(), vecNumType.begin(), vecNumType.end());
    ...
    payload.insert(payload.end(), vecTickSize.begin(), vecTickSize.end());
