...
##################################################################
printf "\n________________________________________\n"
printf "Creating Future Contract #1 with the base address:\n"   #NOTE: Ale's changes: omni_createcontract arguments
TRACreate=$($SRCDIR/litecoin-cli -datadir=$DATADIR --regtest omni_createcontract $ADDRBase 1 4 1 "Future Contract 1" ${blocks_until_expiration} ${notional_size} ${collateral} ${margin_requirement})
$SRCDIR/litecoin-cli -datadir=$DATADIR --regtest generate 1 ## Generating one block
...
##################################################################
# printf "\n________________________________________\n"
# printf "Creating an Divisible Token USDT:\n"   # NOTE: Ale's changes: delete the last argument on omni_sendissuancemanaged
...
#     $SRCDIR/litecoin-cli -datadir=$DATADIR --regtest omni_sendgrant ${ADDRBase} ${ADDRess[$i]} 4 "${amountusdts_manyaddr}" # OMNI = Ecosystem 1
#     $SRCDIR/litecoin-cli -datadir=$DATADIR --regtest generate 1 # Generating one block
