...
std::map<uint32_t,std::map<int,std::vector<arith_uint256>>> mastercore::tokenvwap;
...
int DEx_payment(const uint256& txid, unsigned int vout, const std::string& addressSeller, const std::string& addressBuyer, int64_t amountPaid, int block, uint64_t* nAmended)
{
    ...
    */
    ...
    const arith_uint256 nvwap = (ConvertTo256(unitPrice) * ConvertTo256(amountPurchased))/ ConvertTo256(COIN);
