...
UniValue tl_sendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 3 || !request.params[1].isObject() || request.params[1].size() < 1 || request.params[1].size() > 4)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. fromaddress          (string, required) the address to send from\n"
            "2. \"amounts\"          (string, required) A json object with addresses and amounts (up to 4 pairs)\n"
            "    {\n"
...
// tl_sendltcpayment - send a LTC payment
UniValue tl_sendltcpayment(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 4)
        throw runtime_error(
            "tl_sendltcpayment \"fromaddress\" \"toaddress\" \"linkedtxid\" \"amount\"\n"

            "\nCreate and broadcast a LTC payment transaction.\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("tl_sendltcpayment", "\"3M9qvHKtgARhqcMtM5cRT9VaiDJ5PSfQGY\" \"37FaKponF7zqoMLUjEiko25pDiuVH5YLEa\" \"txid\" \"0.01\"")
            + HelpExampleRpc("tl_sendltcpayment", "\"3M9qvHKtgARhqcMtM5cRT9VaiDJ5PSfQGY\", \"37FaKponF7zqoMLUjEiko25pDiuVH5YLEa\", \"txid\", \"0.01\"")
        );
    ...
    // create a payload for the transaction
    auto payload = CreatePayload_LitecoinPayment(linkedtxid);
    ...
    if (result != 0) {
        ...
        if (!autoCommit) {
            ...
            PendingAdd(txid, fromAddress, MSC_TYPE_LITECOIN_PAYMENT, LTC, referenceAmount);
            return txid.GetHex();
        }
    }
}
...
UniValue tl_sendissuancecrowdsale(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 13)
        throw runtime_error(
            "tl_sendissuancecrowdsale \"fromaddress\" type previousid \"category\" \"subcategory\" \"name\" \"url\" \"data\" propertyiddesired tokensperunit deadline ( earlybonus issuerpercentage )\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("tl_sendissuancecrowdsale", "\"3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo\" 1 0 \"Companies\" \"Bitcoin Mining\" \"Quantum Miner\" \"\" \"\" 2 \"100\" 1483228800 30 2")
            + HelpExampleRpc("tl_sendissuancecrowdsale", "\"3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo\", 1, 0, \"Companies\", \"Bitcoin Mining\", \"Quantum Miner\", \"\", \"\", 2, \"100\", 1483228800, 30, 2")
        );
...
UniValue tl_sendclosecrowdsale(const JSONRPCRequest& request)
{
    ...
    // obtain parameters & info
    const std::string fromAddress = ParseAddress(request.params[0]);
    uint32_t propertyId = ParsePropertyType(request.params[1]);
