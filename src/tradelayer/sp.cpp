...
CMPSPInfo::CMPSPInfo(const fs::path& path, bool fWipe)
{
  ...
  implied_all.issuer = ExodusAddress();
  ...
  implied_tall.issuer = ExodusAddress();
...
void CMPCrowd::saveCrowdSale(std::ofstream& file, CHash256& hasher, const std::string& addr) const
{
    ...
    hasher.Write((const unsigned char*)lineOut.c_str(), lineOut.length());
