...
class TestNode(P2PInterface):
    ...
    def request_headers_and_sync(self, locator, hashstop=0):
        ...
        wait_until(self.received_block_announcement, timeout=60, lock=mininode_lock)
        self.clear_block_announcement()
...
class CompactBlocksTest(BitcoinTestFramework):
    ...
    def test_sendcmpct(self, node, test_node, preferred_version, old_node=None):
        ...
        wait_until(received_sendcmpct, timeout=60, lock=mininode_lock)
        with mininode_lock:
        ...
        def check_announcement_of_new_block(node, peer, predicate):
            ...
            peer.wait_for_block_announcement(block_hash, timeout=60)
    ...
    def test_compactblock_requests(self, node, test_node, version, segwit):
        ...
        for announce in ["inv", "header"]:
            ...
            if announce == "inv":
                ...
                wait_until(lambda: "getheaders" in test_node.last_message, timeout=60, lock=mininode_lock)
                test_node.send_header_for_blocks([block])
            else:
                test_node.send_header_for_blocks([block])
            wait_until(lambda: "getdata" in test_node.last_message, timeout=60, lock=mininode_lock)
    ...
    def test_getblocktxn_handler(self, node, test_node, version):
        ...
        while (current_height >= chain_height - MAX_GETBLOCKTXN_DEPTH):
            ...
            wait_until(lambda: "blocktxn" in test_node.last_message, timeout=60, lock=mininode_lock)
    ...
    def test_compactblocks_not_at_tip(self, node, test_node):
        ...
        wait_until(lambda: "cmpctblock" in test_node.last_message, timeout=60, lock=mininode_lock)
        ...
        with mininode_lock:
            test_node.last_message.pop("block", None)
        test_node.send_message(msg_getdata([CInv(4, int(new_blocks[0], 16))]))
        wait_until(lambda: "block" in test_node.last_message, timeout=60, lock=mininode_lock)
        with mininode_lock:
            test_node.last_message["block"].block.calc_sha256()
    ...
    def test_end_to_end_block_relay(self, node, listeners):
        ...
        for l in listeners:
            wait_until(lambda: l.received_block_announcement(), timeout=60, lock=mininode_lock)
        with mininode_lock:
            for l in listeners:
