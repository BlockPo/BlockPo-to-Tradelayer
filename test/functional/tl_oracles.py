...
class HTTPBasicsTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # self.log.info(params)
        conn.request('POST', '/', '{"method": "tl_create_oraclecontract", "params":'+params+'}', headers)
        resp = conn.getresponse()
        assert_equal(resp.status, 200)
        input = (resp.read().decode('utf-8'))
        out = json.loads(input)
        ...
        self.log.info("Setting oracles prices")
        params = str([addresses[0], "Oracle 1", "602.1", "450.6", "500.1"]).replace("'",'"')
        # self.log.info(params)
        conn.request('POST', '/', '{"method": "tl_setoracle", "params":'+params+'}', headers)
        resp = conn.getresponse()
        # assert_equal(resp.status, 200)
        input = (resp.read().decode('utf-8'))
        out = json.loads(input)
        ...
        # self.log.info(params)
        conn.request('POST', '/', '{"method": "tl_change_oracleref", "params":'+params+'}', headers)
        resp = conn.getresponse()
        assert_equal(resp.status, 200)
        input = (resp.read().decode('utf-8'))
        out = json.loads(input)
        ...
        self.log.info("Checking the admin address in oracle")
        conn.request('POST', '/', '{"method": "tl_getproperty", "params": [5]}', headers)
        resp = conn.getresponse()
        assert_equal(resp.status, 200)
        input = (resp.read().decode('utf-8'))
        ...
        self.log.info("Activating backup address")
        params = str([addresses[2], "Oracle 1"]).replace("'",'"')
        # self.log.info(params)
        conn.request('POST', '/', '{"method": "tl_oraclebackup", "params":'+params+'}', headers)
        resp = conn.getresponse()
        assert_equal(resp.status, 200)
        input = (resp.read().decode('utf-8'))
        out = json.loads(input)
        ...
        self.log.info("Checking now the admin address in oracle")
        conn.request('POST', '/', '{"method": "tl_getproperty", "params": [5]}', headers)
        resp = conn.getresponse()
        assert_equal(resp.status, 200)
        input = (resp.read().decode('utf-8'))
