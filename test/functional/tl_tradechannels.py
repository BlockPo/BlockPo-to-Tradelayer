...
class ChannelsBasicsTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # self.log.info("Creating new addresses")
        # addresses1 = []
        # accounts1 = ["alpha", "beta", "gamma"]
        #
        # addresses1 = tradelayer_createAddresses(accounts1, conn, headers)
        #
        # self.log.info("Getting private keys")
        # params = str([addresses1[0]]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, True, "dumpprivkey",params)
        # # self.log.info(out)
        # assert_equal(out['error'], None)
        # privatekey0 = out['result']
        #
        # params = str([addresses1[1]]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, True, "dumpprivkey",params)
        # # self.log.info(out)
        # assert_equal(out['error'], None)
        # privatekey1 = out['result']
        ...
        #
        # self.log.info("Creating multisig address")
        # params = str([2, [addresses1[0], addresses1[1]]]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, True, "addmultisigaddress",params)
        # # self.log.info(out)
        # assert_equal(out['error'], None)
        # multisig2 = out['result']['address']
        # redeemScript1 = out['result']['redeemScript']
        # self.log.info("multisig:"+multisig2)
        # self.log.info("address0:"+addresses1[0])
        # self.log.info("address1:"+addresses1[1])
        #
        # self.log.info("Checking multisig")
        # params = str([multisig2]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, True, "validateaddress",params)
        # # self.log.info(out)
        # scriptPubKey1 = out['result']['scriptPubKey']
        # assert_equal(out['result']['embedded']['script'],'multisig')
        #
        #
        # self.log.info("Funding addresses with LTC")
        ...
        #
        # self.log.info("Funding address0 and adddress1 with 0.1 LTC")
        # params = str([addresses[0], 0.05]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, False, "sendtoaddress", params)
        # # self.log.info(out)
        #
        # self.nodes[0].generate(1)
        #
        # params = str([addresses[1], 0.05]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, False, "sendtoaddress", params)
        # # self.log.info(out)
        # txid1 = out['result']
        #
        ...
        #
        # self.log.info("Checking the LTC balance in every account")
        # tradelayer_checkingBalance(accounts1, amount, conn, headers)
        #
        # self.log.info("Self Attestation for addresses")
        # tradelayer_selfAttestation(addresses1,conn, headers)
        #
        # self.nodes[0].generate(1)
        #
        # self.log.info("Sending 1000 lihki coins to first address (tl_send)")
        # params = str([addresses[0], addresses1[0], 4, "1000"]).replace("'",'"')
        ...
        # self.nodes[0].generate(1)
        #
        # self.log.info("Sending 1000 dan coins to second address (tl_send)")
        # params = str([addresses[1], addresses1[1], 5, "1000"]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, True, "tl_send",params)
        # # self.log.info(out)
        #
        # self.nodes[0].generate(1)
        ...
        # params = str([addresses1[0], 4]).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, True, "tl_getbalance",params)
        # # self.log.info(out)
        # assert_equal(out['error'], None)
        # assert_equal(out['result']['balance'],'1000.00000000')
        ...
        #
        #
        # self.log.info("Commiting to the new trade channel")
        # params = str([addresses1[0], multisig2, 4, '1000']).replace("'",'"')
        # out = tradelayer_HTTP(conn, headers, False, "tl_commit_tochannel",params)
        # assert_equal(out['error'], None)
        # # self.log.info(out)
        #
        ...
        # # self.log.info(out)
        # assert_equal(out['error'], None)
        # assert_equal(out['result'][0]['sender'], addresses1[0])
        # assert_equal(out['result'][0]['propertyId'], '4')
        # assert_equal(out['result'][0]['amount'], '1000.00000000')
        #
        #
        # # address1[1] wants 1000 lihki coins from channel (address1[0]'s commit)
        # self.log.info("Creating another rawtx transaction")
        # params = '[[{"txid":"'+txid1+'","vout":'+str(vout1)+'}],{"'+addresses1[0]+'":0.01}, 0, true]'
        # out = tradelayer_HTTP(conn, headers, False, "createrawtransaction", params)
        # assert_equal(out['error'], None)
        # hex = out['result']
        # self.log.info(hex)
        #
        # # LTC for Tokens (1 LTC for 1000 lihki coins)
        ...
        # self.log.info(hex)
        # 
        # params = '["'+hex+'",[{"txid":"'+txid1+'","vout":'+str(vout1)+', "scriptPubKey":"'+scriptPubKey+'","redeemScript":"'+redeemScript+'","amount":2}],["'+privatekey0+'"]]'
        # self.log.info("Signing raw transaction with address 0")
        # # self.log.info(params)
        # out = tradelayer_HTTP(conn, headers, False, "signrawtransaction",params)
        # # assert_equal(out['error'], None)
