...
class ChangeAddressTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("address3:"+addresses[3])
        ...
        self.log.info("Adding Change address to transaction")
        prevtxs = [{"txid": txid, "vout": vout, "scriptPubKey": scriptPubKey, "value":2}]
        params = str([hex, prevtxs, addresses[3], 0.5, 1]).replace("'",'"')
        # self.log.info(params)
        out = tradelayer_HTTP(conn, headers, False, "tl_createrawtx_change", params)
        assert_equal(out['error'], None)
        # self.log.info(out)
        hex = out['result']
        ...
        assert_equal(out['result']['referenceaddress'], addresses[3])
