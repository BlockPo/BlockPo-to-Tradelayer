...
class ChannelsCloseTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        params = str([addresses[1], multisig, 5, '3000']).replace("'",'"')
        out = tradelayer_HTTP(conn, headers, False, "tl_commit_tochannel",params)
        assert_equal(out['error'], None)
        # self.log.info(out)
        ...
        out = tradelayer_HTTP(conn, headers, False, "tl_get_channelremaining",params)
        assert_equal(out['error'], None)
        assert_equal(out['result']['channel remaining'], '1875.00000000')
        ...
        params = str([addresses[0], multisig]).replace("'",'"')
        out = tradelayer_HTTP(conn, headers, False, "tl_send_closechannel", params)
        # self.log.info(out)
        tx = out['result']
        self.nodes[0].generate(1)
        ...
        assert_equal(out['result']['fee'], '0.00362000')
        assert_equal(out['result']['sendingaddress'], addresses[0])
        ...
        assert_equal(out['result']['valid'], True)
        assert_equal(out['result']['block'], 221)
